...
modify_makefile() {
    local makefile="${1}"
    if [ -f "${makefile}" ]; then
        sed -i 's/^obj-\(.*\)/obj-m\1/' "${makefile}"
        sed -i 's/obj-m\./obj-m :=/' "${makefile}"
    else
...
build_module() {
    ...
    # Modify the Makefile to build all modules as "m"
    modify_makefile "${module_src}/Makefile"
...
main() {
    ...
    local module_path="drivers/net/can/c_can"
